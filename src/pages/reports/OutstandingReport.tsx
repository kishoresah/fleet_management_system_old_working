...
This is a gentle reminder that the pending balance for your invoice #${inv.lorryNo
      } is ₹${inv.totalPending}.
  
Invoice Date: ${inv.createdAt?.toDate ? inv.createdAt.toDate().toLocaleDateString() : "-"
      }
Last Payment Date: ${inv.lastPaymentDate?.toDate
...
Phone: +91-9540670670`;
  ...
  const sendWhatsAppReminder = (inv: any) => {
    const message = `Hello ${inv.customerName
      }, Pending balance for invoice number ${inv.lorryNo} is ₹${inv.totalPending
      }. Last payment date ${inv.lastPaymentDate?.toDate
        ? inv.lastPaymentDate.toDate().toLocaleDateString()
        : "-"
      } and invoice date ${inv.createdAt?.toDate ? inv.createdAt.toDate().toLocaleDateString() : "-"
